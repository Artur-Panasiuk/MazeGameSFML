...
Player::Player()
	:mMaxVelocity(128.f), mSpeed(2048.f), mBaseFriction(16.f)
{
}
...
void Player::SetTextureManager(TextureManager * lTextureManager)
{
	mTextureManager = lTextureManager;
}
...
void Player::SetTexture(const std::string & lName)
{
	mTexture = lName;
	mTextureManager->RequireResource(lName);
	mSpriteSheet.setTexture(*mTextureManager->GetResource(lName));
	mSpriteSheet.setOrigin(mSpriteSheet.getGlobalBounds().width / 2, mSpriteSheet.getGlobalBounds().height / 2);
	UpdateAABB();
}
...
{
	mSpriteSheet.move(x, y);
	UpdateAABB();
}

void Player::Draw(sf::RenderWindow * lWind)
{
	lWind->draw(mSpriteSheet);
}
...
void Player::Update(const sf::Time & lTime)
{
	...
	mSpriteSheet.move(mVelocity.x*lTime.asSeconds(), mVelocity.y*lTime.asSeconds());
	mAcceleration.x = 0;
	mAcceleration.y = 0;
...
{
	mAABB.left = mSpriteSheet.getGlobalBounds().left;
	mAABB.width = mSpriteSheet.getGlobalBounds().width;
	mAABB.top = mSpriteSheet.getGlobalBounds().top + mSpriteSheet.getGlobalBounds().height / 2;
	mAABB.height = mSpriteSheet.getGlobalBounds().height / 2;
}
